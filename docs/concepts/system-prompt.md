...
LibreClaw Prompt Studio v2 uses the same contribution seam instead of resurrecting removed prompt-replacement config. The initial operator-facing config is `agents.defaults.promptOverlays.openclaw.customInstructions`: when set, it adds a stable `## Custom Instructions` section while preserving the generated OpenClaw prompt, safety section, cache boundary, and provider-owned overlays. Set `agents.defaults.promptOverlays.openclaw.enabled` to `false` to temporarily disable that custom section without deleting the text. LibreClaw exposes this initial v2 editor at **Settings → Prompt Studio** (`/settings/prompt-studio`); it edits only the safe `openclaw.customInstructions` overlay and shows that contributed section, not a full prompt dump.
