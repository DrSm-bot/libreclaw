...
For multi-agent setups where agents need to see each other's messages without triggering runs, enable `channels.discord.historyIncludeBots`. This records other bots' messages to channel history context while still dropping them (no agent run triggered).
